...
- **A new residual trigger, `silenceFlags` entry, or `IgnoredItem` reason is not done until it's a row in `Architect_Residual_Review_Session.md` §3's producer registry.** This is the same class of gap as the two rules above, caught for real: that doc's own top status line once contradicted its own body about whether a Tier B producer existed, because a code change (`triage.py`'s `_TRIGGER_MAP`) landed without the one table an architect reads to know what a Session Pack will ever show them. Applies to `coverage-report.ts`'s `silenceFlags.push` calls, `typed-facts.ts`'s `IgnoredItem.reason` union, and `triage.py`'s `_TRIGGER_MAP` — see that section's own producer table for the exact current inventory.
