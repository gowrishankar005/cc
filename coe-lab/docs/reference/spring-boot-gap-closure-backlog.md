...
| T-CDX-2 | New `scanner/cdxgen-provider.ts` — shells `cdxgen -t java -o bom.json <root>` (and the equivalent for Node/Python roots, matching Weaver's existing polyglot scope), parses the CycloneDX JSON | Real run against a reference Java/JAX-RS banking platform module and the NestJS fixture, both produce a parseable BOM |
