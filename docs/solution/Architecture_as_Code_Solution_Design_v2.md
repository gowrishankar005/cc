...
- The extraction/construction engine is not paper design — it is real, running TypeScript (`pipeline/src/`), verified against real Bank of Anthos services: 4/4 grep-verified routes, 6 real relationships recovered after the accuracy audit, `calm validate` passing with 0 errors/0 warnings.
...
The current selection (CodeGraph primary, Graphify as documented fallback, `CodeGraph_vs_Graphify_Comparison.md` §9) is real, evidence-backed (both tools installed and run against Fineract/OpenBB/Ghostfolio), but was never checked against the wider field. Researched here — **evidence level: researched, not spiked**, unless stated otherwise — before recommending whether to keep the current selection or add to it.
...
| **Regression safety** | Not yet a suite — the enterprise-readiness review is right that only manual runs against BoA + one NestJS fixture exist today. Minimum viable fix, scoped for the next build round: wrap the existing manual verifications (BoA route/relationship counts, NestJS fixture's two regression findings) into an automated `npm test` check that fails the build if a catalogue/mapping-table change silently changes those counts. Not a golden-dataset program — a floor, stated honestly as a floor. |
...
| **Golden-dataset regression program** | §8 commits to a *floor* (automate the existing BoA/NestJS checks so catalogue edits can't silently regress counts), explicitly not a full program. Unchanged, deliberate. |
...
| **Scala/Spark (Fidelity data tier)** | Out of scope by language. Real, evidenced (`v0.14`), named — a Fidelity-shaped estate has a data tier this design cannot see at all. |
