...
### 5.7a k8s-evidenced `database` nodes for deployments with no scanned source — designed 2026-09-02, not yet built
...
**This is the exact same shape `§5.7`'s `system`-node synthesis and `k8s-namespace-node-builder.ts` already solved for a different node kind** — a real, deterministic CALM node built directly from k8s manifest evidence (a Deployment's own container image, e.g. an image literally named `postgres`), never a `TypedUnit`, same pattern already shipped and proven. No LLM involvement — the evidence (a container image name) is as static as any other Scanner-layer fact.

**Not yet built.** Needs: a new builder (e.g. `k8s-database-node-builder.ts`, mirroring `k8s-namespace-node-builder.ts`'s own doc comment and build-order constraint — must run before `relationship-builder.ts`) that recognizes a Deployment whose image matches a small, catalogue-driven list of known database engine images (`postgres`, `mysql`, `mongo`, ...) and synthesizes a `database`-kind node for it when no `TypedUnit` already exists at that name; `env-soft-graph-detector.ts`'s `findUnitForDeployment` call needs to also check this synthesized set, not just real `TypedUnit`s. See `BACKLOG.md` for the tracked row.
