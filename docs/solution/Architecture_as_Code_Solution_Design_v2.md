...
**First persistent solution-design document in this repo** (`docs/solution/` created this round). Supersedes no prior file — the only earlier solution-design material is an external v1 document (not in this repo) and the paused, narrower `scalable-dreaming-hearth` plan (CALM-construction two-catalogue design only — folded into §5 here, not superseded, since its content is still correct, just incomplete relative to the dual-goal scope below).

**Produced against:** the project's requirements history and the real `pipeline/src/` codebase.

---

## 0. Document structure & authority

**Structural decision** (confirmed rather than left implicit): this repository keeps **one platform root + language annexes**, not one monolithic solution document. A full merge would recreate the exact failure mode found once already between this document and the Java companion — contradictory "locked" claims sitting in one file — and would make every Java/Node/Python-specific churn rewrite the platform story unnecessarily.
...
**Goal A status, stated precisely rather than left to a reader's inference:** Goal A (modular platform) is **partially delivered, not fully proven**. What's real: `contractVersion`, a working module registry (`modules/registry.ts`), and a genuine second module (`threat-signals`) consuming only `TypedFacts` — verified directly against real code, not aspirational (§9, §Wave-C-verification below). What's still target architecture, not delivered foundation: module registration is a code edit, not config (Wave M T-M1 closes this); Scanner/orchestration remain a closed recipe, not a plugin bus (Wave M T-M7); no library/API embed surface exists (Wave G, explicitly deferred). Read Goal A as "real boundary, not yet a seamless platform" — both halves of that sentence matter.
...
- Repo topology, mapping-config structure, Decision Record/Override schema, confidence-scoring weights, Ignored Items taxonomy — all drafted with real shapes early in this project's design history.
- The four items the enterprise-readiness review found silently dropped (numeric success metric, data-classification default, companion-interface auth status, Decision Record/Override build path) all got real, non-silent answers in the requirements history — not "resolved" in the sense of being built, but resolved in the sense that no reviewer reading v0.10 forward is misled about their status.
...
These are settled and re-asserted, not re-argued, below: CodeGraph's Spring-only Java coverage and undocumented bulk-export path; `node.decorators`'s falsified generality (`extractFromSource()`'s `decorates` refs are the real mechanism); the `detect()` gate's silent per-root failure; CodeGraph's inability to merge multiple indexed roots; the vertical-slice delivery strategy (Slice 1 Python/Node built, Slice 2 Java scoped, everything else backlog not discarded); `node-type`/interface `type` being open, not closed, enums.

### Assumptions carried into Part B

- CALM governance (pattern validation, org policy authoring, control catalogues as a product) is out of scope by design, confirmed correct by design (capture control *evidence*, not policy). This document does not walk that back.
- The target deployment environment for a first real pilot is still unconfirmed (a still-open data-classification/execution-environment decision) — Part B designs for a default-safe stance (local-first, no shared artefact storage without an explicit owner) rather than waiting on an org decision this project has no authority to make.
...
flowchart TB
    ...
    subgraph Modules["Module Framework — Goal A proof point"]
        ...
        THREAT["threat-signals module<br/>(§9 — BUILT & proven)"]
...
Confidence scoring (`Gap_Closure §7`'s weighted-signal bands, already implemented in `analysis/confidence-scorer.ts`), the Ignored Items taxonomy (implemented, `analysis/ignored-items.ts`, 8 codes including the federated-ownership-driven `CROSS_DOMAIN_UNRESOLVED`), and Decision Records/Overrides (`Gap_Closure §6`'s schema, **built and tested** — `modules/calm-generator/override-applier.ts`, see §5.4) exist to help an architect trust or correct *generation accuracy*. This is explicitly not CALM governance — an architect confirming "yes, this is really a service" is a different activity from "does this architecture satisfy our security pattern," and this design does not conflate them, per the v0.14 prompt's explicit instruction.
...
**Control requirement-url portability — a real handoff constraint, documented not assumed** (and §5.5's `calm-cli` host-allowlist finding). Generated `architecture.calm.json` references control `requirement-url`s as placeholder `https://` identifiers that are *not* independently resolvable — validating with plain `calm validate -a architecture.calm.json` on a machine other than the one that generated the output **will fail** with a host-allowlist error, this is expected, not a bug. **The portable path**: `npm run build` regenerates `dist/rules/control-url-mapping.json` fresh with absolute local paths every time (`scripts/generate-control-url-mapping.js`), and `npm run validate` always passes `-u` pointing at it — so `npm run build && npm run validate -- <output>` on *any* checkout of this repo works without hand-edited paths, confirmed directly (`npm ci` + fresh clone + `npm test`, which exercises this exact path, ran clean). **External consumers who only receive the JSON artefact, not this repo, cannot resolve it the same way** — this is a real, disclosed limitation of the current handoff contract, not yet solved: an external governance system running `calm validate` against a received `architecture.calm.json` needs its own `-u` mapping (or its own copy of `pipeline/src/rules/control-requirements/`) to avoid the same allowlist failure. Named honestly as an open handoff gap, not implied solved by the in-repo fix.
...
`build-calm.ts` (94 lines) is accurate for every case it currently handles, but every one of its decisions is a hardcoded conditional, not a data lookup:
...
### 5.7a k8s-evidenced `database` nodes for deployments with no scanned source — shipped 2026-09-02

**Real gap found reviewing a live scan** (a reference Java microservices banking sample): several services' own outbound `env-soft-graph` evidence (a ConfigMap value naming a database's k8s Service/Deployment, e.g. `SPRING_DATASOURCE_URL` → the real Postgres Deployment's name) correlates correctly once `§7.1`'s eligible-evidence rule is applied — but even then, `env-soft-graph-detector.ts`'s `findUnitForDeployment` still fails, because the database Deployment (e.g. `ledger-db`) is a pure init-script/Postgres directory with **no scanned source at all** — no `TypedUnit` exists for it to attach a relationship to, regardless of how well the ConfigMap value resolves.

**This is the exact same shape `§5.7`'s `system`-node synthesis and `k8s-namespace-node-builder.ts` already solved for a different node kind** — a real, deterministic CALM node built directly from k8s manifest evidence, never a `TypedUnit`, same pattern already shipped and proven. No LLM involvement — the evidence (a container image or container name) is as static as any other Scanner-layer fact.

**Built** (`AGENT_TASKS_K8s_Database_Node_Synthesis.md`): `k8s-database-node-builder.ts` mirrors `k8s-namespace-node-builder.ts` exactly (build-order constraint included). No new catalogue — reuses `contradiction-detector.ts`'s own already-shipped, already-evidenced `imageEngine()` directly. `env-soft-graph-detector.ts`'s target-side lookup falls back, when no `TypedUnit` exists, to checking the deployment's own container image OR container name against a recognized database engine, synthesizing a `k8s-database:` node id instead of guessing.

**Real scoping gap found only by verifying end-to-end against the actual real fixture, not assumed from the design alone**: `k8s-manifest-provider.ts` only ever recognized `kind: Deployment` — the real `ledger-db`/`accounts-db` manifests this section's own motivating evidence cites are `kind: StatefulSet` (a real, common pattern for stateful workloads), so they were never even scanned at all until this was found and fixed (now recognizes both kinds — identical Pod-template shape, not a special case).

**A second real gap, found the same way**: even once scanned, a team's own custom-built database image commonly has NO literal engine name anywhere in its registry path (`ledger-db`'s real image path is `.../bank-of-anthos/ledger-db:v0.6.10@sha256:...`, not `postgres`) — but the container is conventionally still named after its role. Extended to check `containerName` as a second, independent signal after `image`.

**Honestly disclosed, not silently claimed complete**: even with both extensions, THIS specific real fixture's own two originally-cited motivating edges (`ledger-db`, `accounts-db`) still don't resolve end-to-end, each for a different, real reason — see the dated finding in `pipeline/test/regression.test.js`'s own real BOA-fixture test and `BACKLOG.md`'s row for the precise detail. The mechanism itself is proven real and correct (synthetic tests reproduce `ledger-db`'s exact real container-name shape and confirm it resolves) — this repo's own specific naming choices for these two edges just don't happen to trigger it. A third heuristic (e.g. matching a `volumeMounts[].subPath` convention) was deliberately NOT added — a weaker, more indirect signal chasing this one fixture's remaining gap would be exactly the instance-tuned patching this project's own discipline warns against, not a generalizable capability.
...
The current selection (CodeGraph primary, Graphify as documented fallback, documented research) is real, evidence-backed (both tools installed and run against the reference Java/JAX-RS banking platform/OpenBB/the reference Node/NestJS wealth-management app), but was never checked against the wider field. Researched here — **evidence level: researched, not spiked**, unless stated otherwise — before recommending whether to keep the current selection or add to it.
...
- **Decision Records / Overrides**: built and tested (§5.4) — `override-applier.ts`, verified against real output from a reference Python microservices banking app, not just specified.
...
### 7.1 LLM advisory layer — where it goes, why it doesn't touch the no-LLM-core-path guarantee, and how eligibility is actually decided

> **Status (2026-09-03): the mechanism is fully built and proven across six real trigger classes** (`unresolved-outbound-target` T-1+T-2, `low-confidence-emitted-relationship`, `S3-messaging-producer-unverified`, `hand-rolled-resilience-candidate`, plus the pre-existing `S2-http-without-security-control` dossier addendum). The advisory/dossier half of this section (`tools/review-session/advisory.py`, `dossier.py`, `draft_tier_b.py`, `preview_merge.py`) is real, tested, and has run against a live backend — see `tools/review-session/README.md` and `Architect_Residual_Review_Session.md` for the current, authoritative operational detail; this section stays the platform-level statement of *where the layer is allowed to sit* and *what it's allowed to touch*, not a duplicate of that doc's own detail. The **eligibility rule below (originally version-1) has been revised** — see "What changed and why" — closing a real, previously-disclosed gap (`Architect_Residual_Review_Session.md` §3.1's `CROSS_DOMAIN_UNRESOLVED` producer row used to say *"filtered as noise today... a real, disclosed gap if any of these turn out to be architect-relevant... not yet evaluated"* — this section is that evaluation). **Real, disclosed residuals, not silently closed**: `unresolved-outbound-target`'s own real residuals still can't be drafted by `draft_tier_b.py` (a separate gap in the underlying detectors' own detail-string shape, not this layer's own mechanism — see `Architect_Residual_Review_Session.md`'s own status line); T-RT-3 (extending `control-requirement-catalogue.yml` with new evidence) remains separate and not started.

**The question this answers:** the deterministic pipeline has a real, honest failure mode — some units land below the confidence floor (`CONFIDENCE_FLOOR = 40`, `run-slice.ts`), get ignored as `INSUFFICIENT_EVIDENCE`/`AMBIGUOUS_BOUNDARY`, or (the case this revision adds) have real, concrete evidence of intent — a genuine `RestTemplate`/`requests` outbound call, a ConfigMap value shaped like a service address — that a deterministic correlation mechanism simply couldn't resolve to a target. Today the third case is a dead end exactly like the first two: the item sits in `ignored-items-report.json`, never resurfaced. An LLM is a plausible way to give an architect a starting hypothesis instead of a blank page, **without weakening the determinism guarantee this project has held since v0.1** — the reason to be careful about *where* and *what counts as eligible*, not whether to reject the idea outright.

**Where it does not go:** not in `run-slice.ts`'s call graph, not upstream of `typed-facts.json`, not inside any of the five CALM-construction builders (§5). Any of those would make `TypedFacts` — the versioned contract a *second module* (§9) is meant to trust — nondeterministic, which breaks Goal A (a stable platform contract) to patch a Goal B edge case. The project already drew this line once correctly for `suggest-rules.ts` (catalogue-authoring, offline, never imported by the run path); the same discipline applies here, not a new principle. **`architecture.calm.json` itself is never written to by this layer, directly or via auto-merge, regardless of how confident an LLM's suggestion is** — the only legal write path remains Decision Record + Override, applied by `apply.py`'s own explicit-confirmation gate (§5.4), unchanged by anything in this revision.

**Where it goes:** `tools/review-session/` — not a separate, parallel LLM system. Everything this layer does is an extension of the residual-review tooling already built there (`advisory.py`, `dossier.py`, `draft_tier_b.py`, sharing `llm_common.py`'s evidence-assembly and network-boundary logic), reached through the same Session Pack an architect already works from. A second, disconnected "architecture completion" tool would duplicate the evidence-boundary, redaction, and never-a-fact guardrails this one family already has correctly built and tested once.

**Eligibility — what changed, and why:**

The original version of this rule excluded `CROSS_DOMAIN_UNRESOLVED` wholesale, reasoning it was *"an ownership question, not an evidence question."* That's correct for some of what carries that reason code — `unresolved-multi-hop`'s zero-candidate case (nothing to reason from at all) and its 2+-candidate case (evidence exists but doesn't discriminate between equally-plausible options) are genuinely not an LLM's lane, no matter how the prompt is framed. But `CROSS_DOMAIN_UNRESOLVED` is not one shape — it's a single `IgnoredItem.reason` value now covering several genuinely different sub-shapes (see `Architect_Residual_Review_Session.md` §3.1's producer table), and the blanket exclusion was written before that reason code accumulated the ones that don't fit its original reasoning. `tier-b-single-candidate` already carries this same reason code and is already treated as draftable — proof the blanket rule was already inconsistent with how the codebase treats one of its own sub-shapes.

**The eligibility test is now keyed on evidence-specificity, not the reason code:** does this item carry one specific, citable piece of evidence (a literal ConfigMap value, a literal outbound-call site) that a correlation mechanism failed to resolve — or is it a genuine absence/ambiguity with nothing to discriminate on? The first is eligible; the second stays excluded, exactly as originally intended. Concretely, this adds `unresolved-http-target` and `unresolved-env-target` to the eligible set (both already carry a real, specific evidence string — see `Architect_Residual_Review_Session.md`'s new §3.2 for the full producer/Tier/class wiring); `unresolved-multi-hop`'s zero/2+-candidate cases and genuine ownership/boundary questions remain excluded, unchanged.

**Generation may run un-gated over the whole eligible set in one pass — application still never does.** The original design implicitly assumed one architect-facing interaction per suggestion. For evidence-thin, low-per-item-stakes classes specifically, requiring a synchronous human decision *before a suggestion even exists* is friction with no matching safety benefit — nothing is written to CALM by generating a suggestion, only by an architect (or a review agent) later promoting one. `tools/review-session/`'s existing draft mechanism already gives this a natural home: a generated-but-unapplied draft sitting in `drafts/` **is** the "inferred, not yet confirmed" state — no new artefact, no new `x-aac-status` value, no `TypedFacts` contract change. What's new is permission for the drafting step itself to run across an entire eligible batch non-interactively (still never touching CALM), where today's Tier B flow assumes one-at-a-time interactive drafting. The one-at-a-time discipline stays exactly as strict as it is today for every other tier/class — this is a narrower carve-out for evidence-thin, individually-low-stakes items, not a loosening of the general rule (see `Architect_Residual_Review_Session.md` §5 Hard Rule 10, and its own history — Entry 24 — for why that rule exists and stays in force everywhere else).

**Output stays a proposal, never a fact**, and every other original guardrail is unchanged: input bounded to the flagged item's own evidence snippet (never the whole file, never the repo — the concrete defense against an LLM recalling a well-known public repo's real architecture from training data instead of reasoning from *this* run's evidence); `llmConfidence` kept structurally separate from `x-aac-confidence`; non-reproducibility disclosed via `modelUsed`/`promptVersion`, never hidden inside a blended number; the same data-handling/consent-gate stance for sending real source snippets to an external LLM API; the same cost-containment posture (opt-in CLI, never a step inside `run-slice.ts`).
...
| **Vendor risk** | CodeGraph is a real, single-vendor dependency for Slice 1/2's native route typing. Mitigation: version-pin, isolate all engine-specific access behind the Scanner adapter boundary (§3.2), Graphify as the documented (not built-out) fallback, tree-sitter as the researched long-term option (§6). |
...
| **3 — second module / platform proof** | `threat-signals` module (§9) | Quarkus/Spring Data repository-interface/Spring Batch/JMS (evidenced `v0.12`), messaging two-mechanism detection (`v0.11`) | **Platform half built** (`threat-signals` real, proven); construction half not started |
...
### 10.1 Pilot scorecard (provisional numbers)
...
| Control-detection provenance: reuse `x-aac-provenance` metadata, or emit CALM-native `evidence.json` | (a) Fold control evidence into the same `x-aac-*` metadata array every other signal type uses; (b) Emit real `evidence.json`-shaped objects (`{unique-id, evidence-paths, control-config-url}`) for control detections specifically | (b) | CALM has a first-class, schema-defined construct for exactly this evidence category (confirmed directly, not previously known to exist) — using it is a better fit than a namespaced metadata escape hatch built for a project that didn't know the native construct existed. `x-aac-provenance` stays correct for evidence types (routes, persistence) CALM has no dedicated construct for. |
...
| From v1 (Solution Design / Playbook, external, not in this repo) | Harvested or discarded here | Why |
|---|---|---|
...
The project's requirements history; `pipeline/src/types/typed-facts.ts`, `pipeline/src/modules/calm-generator/build-calm.ts`, `pipeline/src/orchestration/run-slice.ts`, `pipeline/src/rules/signal-catalogue.yml` (all read directly, not recalled); tool-comparison research (CodeGraph vs. Graphify baseline); an enterprise-readiness review (source of the four-item and mapping-config drift findings); a fresh `gh api` fetch of the authoritative CALM schema plus real worked examples (`control-requirement.json`/`evidence.json` — §5.5-5.7 of this document are built directly on it); the paused `scalable-dreaming-hearth` plan (harvested into §5, not superseded).
