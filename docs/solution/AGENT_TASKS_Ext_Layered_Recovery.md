...
| **T-LR-6 Per-(engine, fact-type) trust tiers** | **Moved, then done, 2026-08-20** — see `AGENT_TASKS_Ext_CodeQL_Engine.md` (`pipeline/src/analysis/fact-trust-matrix.ts`) | T-LR-5 | Matrix becomes evidence-earned per fact type, not config-declared per framework |
...
the reference Java governance platform's own repo was NOT present in this build environment (checked,
confirmed absent) at initial build — the second-real-repo-instance bar
T-LR-1/T-LR-2 both cleared could not be run for T-LR-3 that day; closed
2026-08-16 once `spikes/` was available (see the T-LR-3 row above and
`Claim_Register.md`'s `R2-mechanism` row for the real reference-governance-platform result), not
silently left open.
...
6. **Second real-repo instance, per `Catalogue_Intake.md`'s bug-fix
   ...
   resolution) are the reference-banking-platform-only. the reference Java governance platform's own repo is already present
   ...
   see the earlier reference-governance-platform genericity probe) — the natural second instance,
   not a new clone to source. Required before either capability counts as
   a mechanism-class fix rather than a reference-platform-instance patch, same bar
