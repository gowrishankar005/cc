...
**Observed:** Architect ran the residual session (genuine VS Code + GitHub Copilot Chat, model shown as GPT-5.6) against a fresh Bank of Anthos pack — a real multi-hop 3-service scan (`spikes/boa/repo/src/ledger/{ledgerwriter,balancereader,transactionhistory}`, `review-sessions/boa-run/`, 23 Tier A residuals). Two attempts:
...
**Verified against real data, not just synthetic tests:** ran `pack.py --with-dossier` against the real Bank of Anthos pack with a real `claude` CLI call. `R-001`'s real, rendered card now shows a correct, evidence-cited "My read" paragraph identifying the real JWT bearer-token verification on `/transactions` (`this.verifier.verify(bearerToken)`) and flagging it as a likely catalogue detection gap — independently matching what a manual evidence read found earlier in this same pilot. Two new regression tests in `test_cards.py` (dossier present / dossier absent) plus a rewritten `test_chatmode_safety.py` assertion pinning that the instruction file no longer asks the model to author anything live.
