...
| Date | Note |
|---|---|
| 2026-08-09 | **Post-close program-wide integrity review found and fixed 3 more real bugs at the integration seams between pieces**, beyond every individual piece's own already-fixed bugs. (1) `draft_tier_b.py`'s write loop had the exact same class of bug already found and fixed in `apply.py`'s `_merge_drafts` — nothing checked whether a model returned the same `decision_id`/`override_id` for two DIFFERENT residuals in one batch, so the second write would silently overwrite the first. Confirmed with a failing repro before fixing; refactored the write loop into a testable `process_tier_b_batch()` (injectable `draft_fn`, no network needed to test) that now tracks written ids and refuses a collision instead of clobbering. (2) `apply.py` silently degraded endpoint-existence checking to "not checked" with zero warning whenever the source scan's `architecture.calm.json` was missing (moved/deleted since the pack was built) — not a safety hole (`override-applier.ts` independently re-checks at real apply time regardless), but a real observability gap; fixed with an explicit warning, and fixed a second bug alongside it — validation warnings were only ever surfaced on the FAILURE path, silently discarded on success, including this new one. Both now printed and recorded in `apply-report.md` regardless of outcome. (3) `residuals-schema.json` never documented the `card` field (added in T-RS1-4, present on every real residual, explicitly load-bearing per the chat-mode file's own instructions) — real doc-drift, now fixed; also refreshed two field descriptions still saying "RS-4 not yet built"/"once built" after RS-4 closed. 3 new regression tests (86 total, up from 83), all real (an injectable-function test for the batch collision, a real end-to-end subprocess test deleting the source CALM for the warning fix). Zero `pipeline/` diff. |
