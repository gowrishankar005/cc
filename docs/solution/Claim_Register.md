...
| **U-persist-import** | Driver/import persistence (SQLAlchemy, Prisma `ref_`, …) | **partial → proven for @prisma/client's ownership shape** (B-ontology, 2026-08-08) | BoA SQLAlchemy; real Ghostfolio re-run: `AccessService` (imports Prisma types only) correctly ZERO database units; `PrismaService` (genuinely `extends PrismaClient`) correctly 1 real unit | L1 with low confidence ok; L1 ownership-precision now real for the one evidenced library | Q13's decided ontology ambiguity is now CODE-FIXED for `@prisma/client` via a new `ownerBaseClass` catalogue field + `class-ownership-resolver.ts` (bounded multi-line source read-back — a real finding falsified Q13's own proposed "require an inherits edge" fix: Graphify emits NO inherits edge for an external base class like `PrismaClient`, confirmed via a real extraction). Every OTHER driver-import library (SQLAlchemy, org.postgresql, org.jooq, TypeORM, Mongoose, …) is UNCHANGED — scoped narrowly to the one library with real evidence, per Q13's own explicit deferral, not applied blanket |
...
| **R2** | Multi-hop / layered (API→app service→entity) | **partial (T-C1 + R2b/T-R1-2 + T-R1-3) — flagship Fineract case CLOSED** | `multi-hop-bridge-detector.ts` (Phase 1 + R2b implementer-import hop) + `java-import-resolver.ts` (T-R1-3, fixes the Java import-target root cause both mechanisms depend on); synthetic fixtures (`r2-bridge-sample`, `r2b-implementer-hop-sample`) + real Fineract `fineract-charge`+`fineract-provider` multi-root, real Fineract `fineract-security`, real Waltz `waltz-data` | L2 PASS for the flagship Fineract shape as of T-R1-3 (see Real evidence). Still L2 fail/inapplicable for: Python-shaped bridges (no structural `implements` edge — named gap, not silently assumed solved), command-bus/dynamic-dispatch write paths (permanent non-goal) | Primary Fineract gap — **now closed for the actual flagship case**, not just the general mechanism. Real, substantial evidence chain across three real repos, in order: (1) R2b (T-R1-2) resolved 11 real relationships in `fineract-provider` + 3 in `fineract-core`, but the specific `ChargesApiResource → Charge` case stayed an honest residual because its bridge implementer (`ChargeReadPlatformServiceImpl`) imported neither the entity nor a catalogue-recognized driver; (2) T-R1-3 found and fixed the actual root cause blocking that recognition — Graphify's Java `imports` edges only ever carry the bare last symbol (`utils`), never the qualified package, so `org.postgresql`/`org.jooq`/`org.springframework.jdbc.core` catalogue rows were silently unreachable regardless of R2/R2b's own logic; fixed via source-line read-back (`java-import-resolver.ts`), verified against real Fineract (`fineract-security`) and real Waltz (`waltz-data`, 229 real jOOQ database units); (3) adding `org.springframework.jdbc.core` as a driver-import row and re-running the real multi-root scan **closes the flagship case**: `ChargeReadPlatformServiceImpl` becomes its own real `database` unit, and `ChargesApiResource → ChargeReadPlatformServiceImpl` is a real, `calm validate`-clean (0 errors), confidence-10, `grade: architecture` relationship — via a Phase 1 mechanism, R2b's extra hop wasn't even needed once the import-resolution fix landed. `architectureOutboundCoverage` for the real combined run: 98.3% (up from 97.5% pre-fix). See `docs/solution/AREC_R2_MultiHop_Strategy.md` (Phase 1), `AREC_R2b_Implementer_Store_Hop.md` (R2b design + evidence), `java-import-resolver.ts` (T-R1-3 root-cause fix) for full detail. |
...
| **S-silence** | Completeness / empty neighborhood loudness | **partial** | BoA: S1 does not fire (R1 present); multi-root Fineract charge+provider: high arch-cov after T-R1-3; single-root residual modules may still S1 | L3 for S0/S1/S2; S3 T-D3; S4 scope-limitations | T-A1 completeness block; T-E5 hitl-review-trigger (S1/S2); T-R0-5 S0 Graphify; T-R0-2 arch-cov rate; **T-R4-1** low-architecture-coverage review trigger (e.g. fineract-security ~17%, 5 services flagged when S1 does not fire) |
...
|---|---|
| “Java relationships work end-to-end for every monorepo shape” | “JAX-RS/JPA units proven; R1 one-hop proven; **R2 flagship multi-root Fineract charge story closed** (T-R1-3); Python multi-hop and command-bus write paths still out/partial” |
...
| “Node persistence fully ontology-correct” | “Import/driver persistence detects units; Prisma service-vs-database ontology still a named limitation (Q13 / **B-ontology**)” |
| “Messaging complete including cloud queues” | “Kafka consumer + field-type producer partial/proven; **SQS/SNS producer still open** (**B-msg-prod-sqs**)” |
...
In short: R0 structural ≠ architecture L2; R2 + C-call (or OOS) before Fineract-class story green. **Update 2026-08-08 (T-R1-3)**: R2's own multi-hop story for the flagship Fineract-charge case (`ChargesApiResource → Charge`-family) is now L2 PASS in a real multi-root run — see the R2 row above for the full evidence chain. C-call (T-D1) remains `partial` (two vocabularies only) — a real relationship existing and a control being detected on the source/target service are still two independent claims; a relationship's existence does not imply its endpoints carry full control coverage. "R2 closed + C-call partial" is a materially different, stronger position than the prior "both partial" state — see `coe-lab/docs/validation-approach-vnext.md`'s expected-fail table for what should be re-run against this.
