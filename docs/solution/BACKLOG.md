...
| **P1-serverless** | Fidelity yardstick + serverless HTTP + Dynamo handler ontology | `doing` — **Y0+Y1+Y2+Y3 done, 2026-08-08**, Y4 (path join) next | **Authority:** [`Fidelity_Yardstick_Closeout_Matrix.md`](./Fidelity_Yardstick_Closeout_Matrix.md). **Agent tasks:** [`AGENT_TASKS_Fidelity_Yardstick_and_Serverless.md`](./AGENT_TASKS_Fidelity_Yardstick_and_Serverless.md) (phases Y0–Y6). Lambda handler → real `service` unit (not invisible, not mis-kinded database) — real fix, locked regression test, 56/56 suite green. Paths still open (Y4, CFN/SAM join). Exam: lab + `aws-saas-boost-tier-service` + `aws-saas-boost-tenant-service`. |
...
| **B-lambda-http** | **Serverless HTTP as first-class entry** (Fidelity cloud): detect Lambda / API Gateway **handlers as `service`** + recover HTTP paths from infra (CFN/SAM/Serverless) and/or OpenAPI — **not** via CodeGraph route resolvers alone | `partial` — **unit-kind done (Y3), paths open (Y4)**, 2026-08-08 | Claim **U-http-serverless** `partial`. Matrix rows **FY-http-lambda**, **FY-infra-cfn**. `implements RequestHandler`/`RequestStreamHandler` → real `service` unit, locked regression test (`java-lambda-apigw` fixture). Java-only; Node/Python deferred (no sample). Tasks: [`AGENT_TASKS_Fidelity_Yardstick_and_Serverless.md`](./AGENT_TASKS_Fidelity_Yardstick_and_Serverless.md) Y4 next. |
