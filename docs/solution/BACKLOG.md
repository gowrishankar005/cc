...
| **P1-serverless** | Fidelity serverless HTTP + Dynamo handler ontology | `todo` — **B-lambda-http**, **B-dynamo-handler-kind** | Hard-test HT-ASB-001/002 promoted to product P1 (not CoE-only). Claim **U-http-serverless** / **U-persist-import** Java Dynamo. Exam: `aws-saas-boost-tier-service`. |
...
| **B-lambda-http** | **Serverless HTTP as first-class entry** (Fidelity cloud): detect Lambda / API Gateway **handlers as `service`** + recover HTTP paths from infra (CFN/SAM/Serverless) and/or OpenAPI — **not** via CodeGraph route resolvers alone | `todo` (promoted 2026-08-08 from CoE HT-ASB-001) | Claim **U-http-serverless** (`specified-unbuilt`). Mechanisms: (1) code entry — `RequestHandler` / handler class / event types (`APIGatewayProxy*`); (2) structured-file — API Gateway path+method → interface join; (3) optional OpenAPI merge (reuse **B-openapi-dual** precedence). Standing exam: `coe-lab` hard-test **aws-saas-boost-tier-service** ([finding](../../coe-lab/docs/findings/aws-saas-boost-tier-service-gold-vs-platform.md), HT-ASB-001…006). Upstream CodeGraph [#1181](https://github.com/colbymchenry/codegraph/issues/1181) only covers C# Lambda *entry marking* — path recovery stays platform work. **Do not** claim Fidelity cloud HTTP complete until this is L1-green on that exam. |
| **B-dynamo-handler-kind** | **Java Dynamo import ≠ store owner** when class is a Lambda/API handler (extends Prisma-style ownership discipline to AWS SDK client imports) | `todo` (promoted 2026-08-08 from CoE HT-ASB-002) | Claim **U-persist-import** (Java Dynamo residual). Real miss: SaaS Boost `TierService` mis-kinded `database` solely for `DynamoDbClient` import while true store is `DynamoTierDataStore`. Pair with **B-lambda-http** so service→db edges grade `architecture`, not structural DB↔DB (HT-ASB-005). Prefer generic ownership/priority rules (handler evidence beats bare Dynamo import), not SaaS-Boost literals. |
