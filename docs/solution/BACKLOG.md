...
| Scale ceiling | **New, real, unmitigated** — a real V8 heap-OOM confirmed at 2,462 files in one root (**B-scale-oom**), reproduced twice, root-caused to the combined-single-pass Graphify graph + non-streaming artefact writes. Largest previously-proven single-root scan was 823 files; the gap between "proven" and "crashes" is now a real, named number, not assumed safe by extrapolation. |
