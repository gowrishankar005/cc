...
| **P1-serverless** | Fidelity yardstick + serverless HTTP + Dynamo handler ontology | `doing` — **Y0-Y4 done, 2026-08-08**, Y5 (completeness/silence + full exam close) next | **Authority:** [`Fidelity_Yardstick_Closeout_Matrix.md`](./Fidelity_Yardstick_Closeout_Matrix.md). **Agent tasks:** [`AGENT_TASKS_Fidelity_Yardstick_and_Serverless.md`](./AGENT_TASKS_Fidelity_Yardstick_and_Serverless.md) (phases Y0–Y6). **Full real-repo win**: `aws-saas-boost-tier-service` re-scanned with its own real CFN templates — `TierService` real `service` unit, 5/5 real paths exact match to gold, real architecture-grade connects to its store. 58/58 suite green. Exam: lab + `aws-saas-boost-tier-service` + `aws-saas-boost-tenant-service`. |
...
| **B-lambda-http** | **Serverless HTTP as first-class entry** (Fidelity cloud): detect Lambda / API Gateway **handlers as `service`** + recover HTTP paths from infra (CFN/SAM/Serverless) and/or OpenAPI — **not** via CodeGraph route resolvers alone | `partial → proven for Java+explicit-CFN`, 2026-08-08 | Claim **U-http-serverless**. Matrix rows **FY-http-lambda**, **FY-infra-cfn**. `implements RequestHandler` → real `service` unit + real CFN Path/Method/Handler join (`cfn-manifest-provider.ts`) — verified on real `aws-saas-boost-tier-service` (5/5 real paths exact match to gold, real architecture edge). Java-only, explicit-CFN-resources only; Node/Python and SAM `Events:` shorthand deferred (no sample / different shape). Tasks: [`AGENT_TASKS_Fidelity_Yardstick_and_Serverless.md`](./AGENT_TASKS_Fidelity_Yardstick_and_Serverless.md) Y5 next. |
