# Architecture-as-Code — Java Solution Design (Slice 2, Baselined Against a Large Financial-Services Organization's Stack)

**Status: Phase 1 approach LOCKED, proven beyond the original spike.** §1.6's route-composition finding and §1.7's full-module the reference Java/JAX-RS banking platform run (823 files, then 6,781 for the cross-package test in `CLAUDE.md`) both hold. The phasing bet (source-only before build-dependent tooling) is now doubly justified: it delivered real routes, real persistence, real controls, *and* real cross-package edges — the last one only by fixing how Graphify itself is invoked (one combined pass per run, not one pass per root; see `CLAUDE.md`'s pipeline-architecture section), not by needing CodeQL/scip-java at all. Phase 2 (CodeQL/scip-java) remains genuinely deferred — nothing found so far has created a new trigger for it. **Still open, unchanged**: Spring Data repository/jOOQ persistence strategies (§3.1's table), OpenAPI ingestion, and full 7,000+-file efficiency validation for the whole pipeline (the 96-second a full run against the reference Java/JAX-RS banking platform number is for the Graphify scan alone, not an end-to-end run at that scale).

**Authority rule** (per `docs/solution/Architecture_as_Code_Solution_Design_v2.md` §0): this document is authoritative for **Slice 2 Java sequencing and Phase 1/2 engine choice**. The platform root (`Architecture_as_Code_Solution_Design_v2.md`) is authoritative for platform contracts and the CALM construction pattern. Where the two conflict, this document wins for Java engine sequencing, and the platform root's `engine-capability-matrix.yml` snippet is kept in sync with this doc, not the reverse.

**Status of the rest:** Solution design for Slice 2 (Java), companion to `Architecture_as_Code_Solution_Design_v2.md` — extends it, does not supersede it. Everything in v2 (two-goal framing, three construct-mapping catalogues, six builders, IR layer, LLM advisory layer, override flow) holds unchanged; this document adds the Java-specific engine strategy, the a large financial-services organization-stack coverage plan, and the efficiency measures the Java target actually needs.

**Baseline:** a large financial-services organization's technology stack is the yardstick — Java/Spring Boot core, MongoDB/PostgreSQL/DynamoDB, SQS/SNS/Kafka messaging, OAuth2, static Swagger/OpenAPI, cloud-native deployment. Evidence repos remain the public proxies (a reference Java/JAX-RS banking platform, a reference Java governance platform, a reference Quarkus service) with a large financial-services organization's stack as the design target.
...
| **Java engine strategy** | **Augment, don't replace** | CodeGraph + Graphify stay primary for Java (Graphify's the reference Java/JAX-RS banking platform-scale timeout is a config/duration matter, confirmed run repeatedly by the team, not a blocker). scip-java and CodeQL are added surgically for the specific gaps where they're clearly better — never as a from-scratch re-platform. |
| **First Java target** | **Public proxies** | the reference Java/JAX-RS banking platform (JAX-RS + JPA + Kafka + Spring Batch), the reference Java governance platform (jOOQ + Spring MVC), a reference Quarkus service (MongoDB + `@Authenticated`) — a large financial-services organization stack as yardstick, no new repo access needed. |
...
**The finding that makes deferral safe — and better:** the mechanisms Phase 1 relies on are the ones this project has actually *proven against real Java source*, while the build-dependent tools are researched-but-never-run. `CodeGraph.extractFromSource(filePath, source)` reads a file's content directly — **no index, no build, no dependency resolution** — and was verified against real the reference Java/JAX-RS banking platform code returning correct annotation names with correct attribution (36 refs for `ChargesApiResource.java`, 48 for `Charge.java`, confirmed directly). So the honest evidence ranking inverts the original §1 framing: **the build-free mechanism is the proven one; CodeQL/scip-java are the speculative ones.** Leading with the proven path is both faster to first result and better justified.

### Phase 1 — source-only Java (zero build dependency)

Everything here works on a bare `git clone`, no compile — status column added since this table previously read as committed scope when several rows are still design-only:

| Capability | Mechanism | Build needed? | Status |
|---|---|---|---|
| JAX-RS routes (`@Path`/`@GET`) | `extractFromSource()` decorates refs + literal-argument composition | **No** | **Built & proven** — 19/19 real the reference Java/JAX-RS banking platform routes, merged into `codegraph-provider.ts`/`jaxrs-route-composer.ts` |
...
| JPA persistence (`@Entity`/`@Table`) | `extractFromSource()` decorates | **No** | **Built & proven** — 36 real `database` nodes, from a real run against the reference Java/JAX-RS banking platform's core module |
...
| Cross-package edges | Graphify whole-run pass + reconciler | **No** | **Built & proven** — 265 real cross-module the reference Java/JAX-RS banking platform edges, see `CLAUDE.md` |
| k8s trust + deployment decorators | Manifest file parse | **No** | **Backlog** — k8s-manifest provider not built |

**Corrected claim** (an earlier version of this line overstated coverage, corrected here): **Phase 1 mechanisms cover every a large financial-services organization concern that maps to one of the four proven mechanisms (native typing, decorates-ref extraction, import-edge detection, structured-file ingestion) — this is not the same as saying the entire a large financial-services organization stack is already delivered.** Per the status column above, JAX-RS routes, JPA `@Entity`, cross-package edges, and `@PreAuthorize` controls are genuinely **built and proven**; Spring Data, jOOQ, multi-database persistence, resiliency, messaging, OpenAPI, and k8s are **backlog** — designed, not coded. Treat §2's full coverage table below as a *map of what Phase 1's mechanisms could reach*, not a single-slice commitment.
...
**Run directly, against real the reference Java/JAX-RS banking platform source, not simulated.** The open question from §7's original risk table — can `extractFromSource()`'s output actually be composed into correct full route paths — is answered: **yes, with one necessary addition.**
...
- **Result: 19/19 real routes correctly assembled across three real the reference Java/JAX-RS banking platform resource files** (`ChargesApiResource` 6/6, `SchedulerApiResource` 2/2, `DelinquencyApiResource` 8/8, plus 3 further real routes in the same file initially left out of the transcribed ground truth and confirmed correct on recheck — not extraction errors, a ground-truth transcription gap, corrected). Zero mismatches, zero false positives.
...
### 1.7 Merged and run end-to-end against real the reference Java/JAX-RS banking platform — RESOLVED, two more real bugs found

**Not just the spike — the fix is in `pipeline/src/` and a real module from a reference Java/JAX-RS banking platform ran through the whole pipeline** (the reference platform's charge module: `ChargesApiResource` + the `Charge` JPA entity; later the reference platform's core module, 823 files), producing real `architecture.calm.json`, passing `calm validate` at 0 errors. Two pre-existing bugs surfaced, neither visible until Java code actually exercised these paths (full detail in `CLAUDE.md`'s pipeline-architecture section):
...
routes:
  - language: java
    ...
    crossPackageEngine: graphify                  # PHASE 1 — proven, 265 real cross-module the reference Java/JAX-RS banking platform edges
  ...
  - language: java
    ...
    primaryEngine: codegraph-extract-from-source   # PHASE 1 — PROVEN: 19/19 real the reference Java/JAX-RS banking platform routes, merged into codebase
    augmentEngine: codeql                           # PHASE 2 — only if a measured gap appears; none found yet
  - language: java
    framework: [jpa, spring-data-repository]
    primaryEngine: codegraph-extract-from-source   # PHASE 1 — proven for bare @Entity (36 real the reference Java/JAX-RS banking platform nodes); Spring Data repository shape still backlog
...
- **CodeGraph `extractFromSource()` — kept as the proven workhorse for the long tail of Spring annotations** (Security/OAuth2, resilience4j, Batch, Kafka) that neither CodeGraph-native nor a CodeQL query pack covers. This is the mechanism already proven at 36–48 refs/file against real the reference Java/JAX-RS banking platform source, and now also proven for control detection (`@PreAuthorize`); it doesn't get thrown away, it gets *narrowed* to where it's genuinely needed (§4 efficiency — not run on every file).

- **jQAssistant — named, viable now (full build), but held as documented backlog, not adopted this round.** With the build environment it's finally runnable, and its Neo4j-backed JAX-RS/CDI plugins are purpose-built for enterprise Java. But adopting it means a Neo4j integration and a second graph store to reconcile — real cost. Recommendation: **spike it against the reference Java/JAX-RS banking platform during Slice 2b/2c to compare its JAX-RS output quality against Phase 1's proven mechanism**, and adopt only if it clearly beats what's already working. Not a blind add.

## 2. Large Financial-Services-Organization-Stack Coverage — every concern mapped to a mechanism and an engine

This is the concrete "does it cover a real large-financial-services-organization-shaped Java service" table — **read as a coverage map, not a single-slice commitment** (per the correction in §1.5). Each row names the detection mechanism, the engine, the CALM construct it produces, the honest evidence level, and now a **Status** column distinguishing what's actually in `pipeline/src/` from what's designed.

| a large financial-services organization concern | Detection mechanism | Engine | CALM output | Evidence level | Status |
|---|---|---|---|---|---|
...
| **JAX-RS routes** (the reference Java/JAX-RS banking platform, Quarkus) | `@Path`+`@GET` decorates + composition | **extractFromSource primary (Phase 1)**; CodeQL Phase-2 augment only if a measured gap appears | `service` node + interface | **Proven — 19/19 real routes, §1.6** | **built** |
| **JPA persistence** | `@Entity`/`@Table`/`@Column` | extractFromSource decorates | `database` node | Proven; CodeGraph gives zero native (confirmed twice) | **built** — 36 real the reference Java/JAX-RS banking platform nodes |
| **Spring Data repositories** | `@Repository extends CrudRepository/JpaRepository` | extractFromSource, Phase 1 | `database` node (repository-interface pattern) | Evidenced (the reference Python app `v0.12`), mechanism specified | **backlog** |
| **jOOQ persistence** | `org.jooq.*` import + typed-field | Graphify import-edge (like SQLAlchemy) | `database` node | Evidenced (the reference Java governance platform `GenericSelector.java:23-24`) | **backlog** |
| **MongoDB / PostgreSQL / DynamoDB** | Driver/SDK import detection | Graphify import-edge + persistence-detector (generalized, §3.1) | `database` node, one per store | Mongo evidenced (a reference Quarkus service); Postgres/Dynamo a large financial-services organization-named | **backlog** — Java entries not in `PERSISTENCE_LIBRARIES` yet |
| **OAuth2 / Spring Security** | `securityScheme` in static OpenAPI **+** `@PreAuthorize`/`@Secured`/`@EnableWebSecurity` decorates | api-contract provider (not built) + extractFromSource (built) | `controls` (security domain) + `evidence.json`-shaped provenance | `@PreAuthorize` proven (the reference Java/JAX-RS banking platform `DatatableWriteService.java`); OpenAPI-as-signal designed | **partial** — decorator path built, OpenAPI path backlog |
| **Quarkus `@Authenticated`** | decorates ref | extractFromSource | `controls` | Evidenced (a reference Quarkus service) | **backlog** — no catalogue row yet |
...
| **Spring Batch** | `@EnableBatchIntegration`/`@Scheduled(cron=...)` | extractFromSource, cron → `units.json` cron-expression | `service` node (batch-job kind) + timing metadata | Evidenced (the reference Java/JAX-RS banking platform 28 files, `v0.12`) | **backlog** |
| **Kafka** (`@KafkaListener`/`KafkaTemplate`) | decorator (consumer) + typed-field (producer) | extractFromSource + Graphify | `network` node (topic) + `connects` | Evidenced (the reference Java/JAX-RS banking platform, `v0.11` two-mechanism design) | **backlog** |
| **JMS/ActiveMQ** | `JmsTemplate`/`jakarta.jms.Queue` | extractFromSource + Graphify | `network` node + `connects` | Evidenced (the reference Java/JAX-RS banking platform, `v0.12`) | **backlog** |
| **AWS SQS/SNS** | `@aws-sdk`/SDK client import | Graphify import + persistence-detector-style | `network` node + `connects` | a large financial-services organization-named (`v0.14`) | **backlog** |
...
**The problem, restated:** `persistence-detector.ts` assumes one wrapper-class-per-file (`source_location === 'L1'`, single-level `contains` traversal). Java persistence has at least four genuinely different code shapes — JPA `@Entity` classes, Spring Data repository *interfaces*, jOOQ typed queries, plain JDBC — none matching the reference Python app's `db.py` shape. Left as-is, each new shape is a code patch. **This is now also tracked as Wave M task T-M9 (persistence strategy catalogue + dispatcher) — implementation, when it happens, should satisfy both this section and that task's acceptance criteria.**
...
"Add a Java framework" touches at most: (a) a `signal-catalogue.yml` row, (b) an `engine-capability-matrix.yml` routing row, (c) possibly a `persistence-detection-catalogue.yml` strategy if it's a persistence tech. No builder code, no extraction code, unless it needs a genuinely new *mechanism* (a fifth beyond native/decorates/import/structured-file) — which the a large financial-services organization stack, mapped in §2, never does. This bound is the concrete answer to "does it patch per repo": for everything in a large financial-services organization's stack, no — though as §2's status column shows, most rows still need their catalogue entries written, not just the mechanism to exist in principle.

### 3.3 Reconciler heuristics — Graphify now works for Java, scip-java remains a Phase 2 precision option

The filePath+line-range reconciler is the design's most shape-fragile component. **Update**: as of the cross-package fix (`CLAUDE.md`), Graphify's reconciler now produces real, correct cross-module edges for Java (265 real edges, the reference platform's charge+core modules) — the fix was running one combined extraction pass, not swapping engines. scip-java's precise symbol resolution remains a real Phase 2 option if a specific measured gap appears (missed or mis-attributed edges at larger scale), but is not currently required to make cross-package Java detection work at all, which is a stronger starting position than this section originally assumed.
...
- **Graphify's own incremental cache is now used, not discarded every run** (`CLAUDE.md`) — a persistent `.graphify-cache/` replaced the old temp-dir-then-delete pattern. Measured: re-running the reference Java/JAX-RS banking platform's core module went from 8.5s (cold) to 2.7s (cached), a real 3.1x speedup, byte-identical output. This closes the "incremental/caching" item this section originally left as backlog.
...
**Renamed from "governance evidence" — this is evidence-capture, not governance, and the title should say so plainly rather than risk re-conflating the two, the exact scope slip `v0.10`'s controls correction already had to fix once.** Ties directly to v2 §5.5's control-builder and the CALM construct deep-dive:
...
## 6. Validation Plan — against the three proxies, a large financial-services organization's stack as yardstick

1. **the reference Java/JAX-RS banking platform** — the primary Java proxy: JAX-RS routes (proven), JPA persistence (proven), `@PreAuthorize` controls (proven), Kafka messaging (backlog), Spring Batch (backlog). Cross-package edges also now proven (265 real edges).
2. **the reference Java governance platform** — jOOQ persistence strategy (§3.1, backlog), Spring MVC routes (unverified, §2).
3. **A reference Quarkus service** — Quarkus/JAX-RS, MongoDB, `@Authenticated` controls (backlog), and (if it ships one) a static OpenAPI spec.
...
6. **Phase 1 gap measurement, feeding the Phase 2 decision** — measured directly: 265 real cross-package edges found, zero missed/mis-attributed cases identified so far. No trigger for scip-java yet.
...
| Graphify same-package edge under-resolution at wide scan scope | **New, disclosed here** (`CLAUDE.md`) — a real, measured trade-off of the cross-package fix: Graphify occasionally emits a dangling edge-target id at wider scan scope, reducing same-package precision slightly. Named here since this belongs in a risk table, not only buried in `CLAUDE.md`. |
...
`Architecture_as_Code_Solution_Design_v2.md` (parent, platform-contracts-authoritative); the project's requirements history (Java framework + large-financial-services-organization evidence); a large financial-services organization's public technology footprint; real evidence repos (a reference Java/JAX-RS banking platform, a reference Java governance platform, and a reference Quarkus service); `CLAUDE.md`'s pipeline-architecture section (real run results across multiple runs). scip-java and CodeQL: researched, no Phase 2 trigger fired yet — evidence level stated honestly as researched-not-yet-run for both.
