# Architecture-as-Code — Java Solution Design (Slice 2, a large financial-services organization-Baselined)
...
**Baseline:** a large financial-services organization' technology stack (`docs/requirements/CALM_Generator_Requirements_v0_14.md`, `memory/fidelity_fintech_evidence.md`) is the yardstick — Java/Spring Boot core, MongoDB/PostgreSQL/DynamoDB, SQS/SNS/Kafka messaging, OAuth2, static Swagger/OpenAPI, cloud-native deployment. Evidence repos remain the public proxies (the reference Java/JAX-RS banking platform, the reference Java governance platform, CALM Hub) with a large financial-services organization's stack as the design target.
...
| **First Java target** | **Public proxies** | the reference Java/JAX-RS banking platform (JAX-RS + JPA + Kafka + Spring Batch), the reference Java governance platform (jOOQ + Spring MVC), CALM Hub (Quarkus + MongoDB + `@Authenticated`) — a large financial-services organization stack as yardstick, no new repo access needed. |
...
| JPA persistence (`@Entity`/`@Table`) | `extractFromSource()` decorates | **No** | **Built & proven** — 36 real `database` nodes, real the reference Java/JAX-RS banking platform `fineract-core` run |
...
**Run this session, against real the reference Java/JAX-RS banking platform source, not simulated.** The open question from §7's original risk table — can `extractFromSource()`'s output actually be composed into correct full route paths — is answered: **yes, with one necessary addition.**
...
| **MongoDB / PostgreSQL / DynamoDB** | Driver/SDK import detection | Graphify import-edge + persistence-detector (generalized, §3.1) | `database` node, one per store | Mongo evidenced (CALM Hub); Postgres/Dynamo a large financial-services organization-named | **backlog** — Java entries not in `PERSISTENCE_LIBRARIES` yet |
...
- **Graphify's own incremental cache is now used, not discarded every run** (`CLAUDE.md`) — a persistent `.graphify-cache/` replaced the old temp-dir-then-delete pattern. Measured: re-running the reference Java/JAX-RS banking platform `fineract-core` went from 8.5s (cold) to 2.7s (cached), a real 3.1x speedup, byte-identical output. This closes the "incremental/caching" item this section originally left as backlog.
...
## 6. Validation Plan — against the three proxies, a large financial-services organization stack as yardstick
