...
| JPA persistence (`@Entity`/`@Table`) | `extractFromSource()` decorates | **No** | **Built & proven** — 36 real `database` nodes, from a real fineract-core run (a reference Java/JAX-RS banking platform) |
...
- **Graphify's own incremental cache is now used, not discarded every run** (`CLAUDE.md`) — a persistent `.graphify-cache/` replaced the old temp-dir-then-delete pattern. Measured: re-running fineract-core (a reference Java/JAX-RS banking platform) went from 8.5s (cold) to 2.7s (cached), a real 3.1x speedup, byte-identical output. This closes the "incremental/caching" item this section originally left as backlog.
...
`Architecture_as_Code_Solution_Design_v2.md` (parent, platform-contracts-authoritative); the project's requirements history (Java framework + large-financial-services-organization evidence); a large financial-services organization's public technology footprint; real evidence repos (Fineract-shaped, Waltz-shaped, and a reference Quarkus service); `CLAUDE.md`'s pipeline-architecture section (real run results across multiple runs). scip-java and CodeQL: researched, no Phase 2 trigger fired yet — evidence level stated honestly as researched-not-yet-run for both.
