...
| **6** | `AGENT_TASKS_Ext_MultiRepo_Deployment.md` | **T-MR-3 done, T-MR-5 done (both 2026-08-20)** — `deployed-in` runtime placement + boundary-change overrides, see `Claim_Register.md`'s `T-MR-3-k8s-deployed-in`/`T-MR-5-boundary-override` rows | **T-MR-1/2** after T-CL-2 (T-CL-2 is done — this is a deliberate scope cut for the T-MR-3/5 session, not a blocked dependency). **T-MR-4** after CodeQL or as a careful catalogue-only pass | BR-10 residual; **BR-20** later |
