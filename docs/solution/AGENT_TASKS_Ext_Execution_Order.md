...
| **6** | `AGENT_TASKS_Ext_MultiRepo_Deployment.md` | **T-MR-3, T-MR-5, T-MR-1, T-MR-2 all done (2026-08-20)** — `deployed-in` runtime placement, boundary-change overrides, per-repo manifest schema, and ranked cross-repo joins, see `Claim_Register.md`'s `T-MR-3-k8s-deployed-in`/`T-MR-5-boundary-override`/`T-MR-1-repo-manifest`/`T-MR-2-cross-repo-join` rows | **T-MR-4** after CodeQL or as a careful catalogue-only pass | BR-10 residual; **BR-20 now largely closed** (see Business_Requirements_Coverage.md) |
...
Do not open T-LM-6/7 before their gate. T-MR-1/2 are done (2026-08-20), no longer gated.
