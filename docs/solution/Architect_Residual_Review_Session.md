...
**A third kind of producer, not yet built (§3.3): not an `IgnoredItem` at all.** Every producer above reads from `ignored-items-report.json` — a fact that was refused. `low-confidence-emitted-relationship` (§3.3) is different in kind: it reads `typed-facts.json`'s own `TypedRelationship[]` directly, for a fact that *was* emitted, at low confidence. Named here so the registry stays complete once built, not because it exists today.
...
**Not yet built.** Needs: a new residual producer — any `TypedRelationship` with `grade: 'architecture'` and `confidence` below a stated threshold (candidate: the existing `CONFIDENCE_FLOOR = 40` reused, or a lower project-specific bar since 20 is already `env-soft-graph`'s own fixed ceiling) becomes a residual, `hitl-review-trigger.ts`-side; a new Tier A card showing the literal evidence that produced it (the ConfigMap key/value pair) and offering `{confirm — this relationship is real, reject — remove it, leave open}`; on reject, a `relationship_remove` Override (already fully implemented in `override-applier.ts`, §3 Tier B table above). See `BACKLOG.md` for the tracked row.
