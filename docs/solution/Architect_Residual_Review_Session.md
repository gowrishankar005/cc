...
|---|---|---|
| `INSUFFICIENT_EVIDENCE` | `ignored-items.ts` (`ignoreUnknownSignal`, `ignoreLowConfidence`), `resilience-call-detector.ts` (new, 2026-09-02) | Yes — ambiguous residual (generic), or the dedicated `hand-rolled-resilience-candidate` Tier C class for `resilience-call-detector.ts`'s own prefixed detail (explicitly excluded from the generic leftover path in `triage.py`'s `_residuals_from_ignored`, same convention `_CONTRADICTION_PREFIX` already uses, to avoid double-counting the same evidence twice) |
...
| `low-confidence-emitted-relationship` (new, 2026-09-02, shipped — see below) | A | low-confidence-emitted-relationship |
| `hand-rolled-resilience-candidate` (new, 2026-09-02, shipped — see below) | C | hand-rolled-resilience-candidate |
...
**A third kind of producer (§3.3): not an `IgnoredItem` at all.** Every other producer above reads from `ignored-items-report.json` — a fact that was refused. `low-confidence-emitted-relationship` (§3.3) is different in kind: it reads `typed-facts.json`'s own `TypedRelationship[]` directly, for a fact that *was* emitted, at low confidence (`hitl-review-trigger.ts`'s own always-on block, filtering `grade === 'architecture' && confidence !== undefined && confidence < CONFIDENCE_FLOOR`).
...
- **T-2, shipped 2026-09-03** (`AGENT_TASKS_Unresolved_Outbound_Target_T2_Preview_Merge.md`): a new `tools/review-session/preview_merge.py` — canonical `architecture.calm.json` plus the current `drafts/` directory's pending `relationship_add` overrides, rendered as one merged CALM document with drafted-but-unconfirmed relationships tagged distinctly (`x-aac-draft: true` metadata — deliberately not a `FactStatus`/`x-aac-status` value, since these relationships don't exist in `typed-facts.json`'s deterministic sense; they're synthesized purely for preview) so an architect can see the *complete*, semantically-marked picture before confirming anything, not just a text list of pending drafts. Read-only, never touches the canonical file or `drafts/` itself, written to a new `preview-merged.calm.json` file. **Real mechanism reuse, not a second merge implementation**: shells out to the exact same `run-slice.js --from-facts --overrides --out` invocation `apply.py` already uses for a real apply (reusing its own `_merge_drafts` helper directly), just pointed at a throwaway temp `--out` directory instead of the real one. **Two real findings from tracing the actual code**: `override-applier.ts`'s `relationship_add` case unconditionally tags a new relationship with `x-aac-status: reviewed`/`x-aac-override-provenance` — correct for a real apply, a false claim in a preview, so this tool strips both and adds `x-aac-draft: true` instead; `apply.py` never archives a draft after applying it, so "is this override in `drafts/`" is not a reliable "pending" signal — the only correct signal is diffing the merged-temp output against the real canonical file's own relationship set. **Verified**: a zero-overrides sanity check (the diff baseline must reproduce the real canonical relationship set exactly), a real drafted relationship correctly tagged and stripped of real-apply-only metadata, an already-applied override correctly NOT re-tagged as pending, and a real end-to-end run against the same real ledger+accounts scan T-1 was verified against — `calm validate` reports 0 errors on the resulting preview document.

### 3.3 `low-confidence-emitted-relationship` — a genuinely different problem: confirming an already-emitted fact, not proposing a new one

**Found in the same review pass, not folded into §3.2 because it's a structurally different shape.** `§3.2` is about a relationship that was *never* emitted (refused, sitting only in `ignored-items-report.json`). This is about a relationship that's **already sitting in the canonical `architecture.calm.json` today**, at genuinely low confidence — the `env-soft-graph` mechanism (`--enable-env-soft-graph`, opt-in, already documented as low/fixed confidence — `scope-limitations.yml`'s `env-relationship-name-correlation-only`) emits real, name-correlated `connects` relationships at a fixed confidence of 20, and today **nothing surfaces them for a second look, ever** — they ship exactly as trusted as any other `architecture`-grade fact. Confirmed real, not hypothetical: a live scan produced 6 such relationships, 2 of which (`ledgerwriter→contacts`, `ledgerwriter→transactionhistory`) don't correspond to anything in the real system.

**This is a review gap independent of anything §3.2 fixes** — even a perfect `unresolved-outbound-target` implementation does nothing for a fact that already exists. **Tier A, not Tier B**: there's no new candidate to draft, only a confirm/reject decision on something already claimed — matches `contradicting-evidence`'s own shape (an architect judgment call, not a draftable action) more than a single-candidate draft.

**Shipped 2026-09-02** (`AGENT_TASKS_Low_Confidence_Emitted_Relationship.md`). `hitl-review-trigger.ts` gained an always-on block reading `facts.relationships` directly (never an `IgnoredItem`) for any `TypedRelationship` with `grade === 'architecture'` and `confidence !== undefined && confidence < CONFIDENCE_FLOOR` (the existing `40` constant reused, imported from `analysis/passes.ts` — since `env-soft-graph`'s own fixed ceiling is 20, this correctly fires for every relationship that mechanism emits today). `triage.py`'s `_TRIGGER_MAP` routes it to Tier A. `TypedRelationship` gained a small additive `evidenceNote?: string` field (tier (b), `Contract_Evolution_Policy.md` §2(b), no `CONTRACT_VERSION` bump, same precedent as the existing `confidence`/`grade`/`mechanism` fields) — set only by `env-soft-graph-detector.ts` today with the literal ConfigMap name/key/basename that produced the correlation, so the card can show real evidence instead of a bare confidence number. `cards.py`'s new `low-confidence-emitted-relationship` template offers exactly `{confirm — this relationship is real, reject — remove it}` (leave-open/other auto-appended, same convention as every other class). **Neither option needed new apply-side mechanism**: confirm is a Decision Record with no Override at all (`final_decision: {"action": "accepted", "new_value": null}` — already the documented, most-common real outcome); reject is a `relationship_remove` Override, already fully supported (`target_type: 'relationship'` was already a valid, validated target — confirmed before assuming a change was needed). Since no existing residual/card/schema path keyed off a relationship id before this class, the relationship's own `id` (reused directly as the CALM `unique-id`) and its `to` unit are embedded in the free-text `rationale` — the same convention every other producer already uses for similar detail, not a new schema field.
...
| Date | Note |
|---|---|
| 2026-09-03 | **§3.2's T-2 (preview/merge tool) shipped** (`AGENT_TASKS_Unresolved_Outbound_Target_T2_Preview_Merge.md`) — the last open item in the entire 2026-09-02 LLM-assist consolidated plan. `preview_merge.py` reuses `apply.py`'s own real merge/apply mechanism unmodified (`_merge_drafts` + the same `run-slice.js --from-facts --overrides --out` invocation), pointed at a throwaway temp dir instead of the real one, then diffs the result against the real canonical file. Two real findings, not assumed: the real apply path's own metadata tagging (`x-aac-status: reviewed`) would be a false claim if reused unmodified in a preview, so it's stripped and replaced with `x-aac-draft: true`; `apply.py` never archives a draft after applying it, so `drafts/` presence alone is not a valid "still pending" signal — only a diff against the real canonical relationship set is. Verified with a zero-overrides sanity check, a real drafted-and-tagged relationship, an already-applied-override-correctly-not-retagged case, and a real end-to-end run against the same real ledger+accounts scan T-1 was verified against, `calm validate` clean. Scoped narrowly to `relationship_add`-sourced new relationships, matching §3.2's own stated scope — other override types sitting in the same `drafts/` dir are merged in by the same mechanism but not specially tagged, a disclosed limitation. |
| 2026-09-03 | **`hand-rolled-resilience-candidate` shipped** (`AGENT_TASKS_HandRolled_Resilience_Detection.md`) — the last item in the 2026-09-02 LLM-assist batch, and the only one with no real evidence to design against until a dedicated evidence pass was run first. Found real `Thread.sleep` hits in two independent repos: a genuine, textbook hand-rolled exponential-backoff retry (`Sender.java`, confirmed reading the real code — `attempt`/`backoff`/`retries` variables, backoff doubling, the method's own javadoc says "exponential back-off to retry"), and an identical "loop + try/catch + sleep" shape that is actually a polling loop (`LedgerReader.java`) — real, confirmed proof the same deterministic shape covers genuinely different intents. New `resilience-call-detector.ts` emits a real `IgnoredItem` directly, deliberately never routed through `signal-catalogue.yml`/`mapSignalsToUnits` (would either fabricate a phantom node or never surface the real evidence, depending on weight) — never a `TypedUnit`, by construction. Tier C, informational-only card options, `dossier.py` addendum matching the S2/S3 pattern. **Two real, live-found bugs fixed during verification**: `triage.py`'s `_evidence_refs_for` stub (already tracked in BACKLOG.md from `unresolved-outbound-target`'s own T-1) was confirmed blocking this feature too — fixed generically, not narrowly; `llm_common.py`'s `build_evidence_prompt` silently dropped snippets `pack.py` had already captured for no-unit residuals — fixed with a direct lookup. **Real live-model verification**: the model correctly identified `LedgerReader.java` as a polling loop, not retry/backoff, matching the real code exactly — the precise disambiguation this design exists to deliver. `Sender.java`'s response honestly declined to fully judge, correctly noting its evidence window only shows the small `sleep()` wrapper, not the real retry loop elsewhere in the file — a genuine, disclosed secondary finding, not a design defect. 206/206 pipeline tests, 253/253 review-session tests. |
| 2026-09-02 | **§3.3's `low-confidence-emitted-relationship` shipped** (`AGENT_TASKS_Low_Confidence_Emitted_Relationship.md`) — real scoping found this wasn't a simple wire-up: no existing residual/card/schema path keyed off a relationship id before this class (resolved by embedding the relationship's own id in free-text `rationale`, not a new schema field), and `env-soft-graph-detector.ts`'s emitted relationships carried zero evidence (resolved with a small additive `TypedRelationship.evidenceNote?` field, same precedent as `confidence`/`grade`/`mechanism`). `hitl-review-trigger.ts` reads `facts.relationships` directly (a third kind of producer, never an `IgnoredItem`) for `grade === 'architecture' && confidence < CONFIDENCE_FLOOR`. `cards.py` offers confirm (Decision Record, no Override — already the documented common case) / reject (`relationship_remove`, already fully supported, zero new apply-side code). New tests in `regression.test.js` (fires correctly, undefined confidence never coerced to low, non-architecture grade excluded), `test_triage.py`, `test_cards.py` (a real rendered card, not just `build_options`'s return value — the exact depth of check T-1's own review found missing the first time). |
| 2026-09-02 | **§3.2's `unresolved-outbound-target` T-1 shipped** (`AGENT_TASKS_Unresolved_Outbound_Target.md`) — `hitl-review-trigger.ts` now emits this trigger for `unresolved-http-target:`/`unresolved-env-target:`-prefixed `CROSS_DOMAIN_UNRESOLVED` ignored items; `triage.py`'s `_TRIGGER_MAP` routes it to Tier B. Batch/non-interactive drafting needed no new code — `draft_tier_b.py`'s existing `main()` already sweeps every open Tier B residual non-interactively; this class flows through automatically, closing that part of §3.2's "New" bullet as already-true rather than newly-built. **Real end-to-end verification** against the real ledger+accounts services: `review-queue.json` correctly names the 3 controllers behind the originally-missing edges. **Real finding surfaced, not silently absorbed**: all 14 real residuals in that run came back `bar-not-met` — traced to `triage.py`'s `_evidence_refs_for` being a permanent stub (`return []`) that predates this task and affects every Tier B trigger, not just this one; the model correctly refused to fabricate rather than guess, but real Tier B drafting has likely never worked against a live repo for this reason. New `BACKLOG.md` row opened for that separate gap. T-2 (the preview/merge tool §3.2 names as not-yet-built) remains open, unaffected by T-1. |
| 2026-09-02 | **`dossier.py`'s system prompt gets a trigger-specific addendum for `S2-http-without-security-control`** (`AGENT_TASKS_Dossier_Uncatalogued_Auth.md`, `BACKLOG.md`'s "Call-site security-control dossier for uncatalogued auth patterns" row) — no new mechanism, tier, schema field, or CLI flag; the existing opt-in `--with-dossier` pass now specifically asks whether the code shows a real auth pattern this pipeline's 4 catalogued vocabularies don't recognize, instead of a generic evidence summary, when it's run on this one trigger. Verified against a real live `claude` CLI run on the checked-in NestJS fixture: the model correctly named what it looked for (`@UseGuards`, guard/interceptor imports, manual checks) and correctly reported none present rather than fabricating a pattern. Not T-RT-3 (`AGENT_TASKS_Ext_Review_Throughput.md`) — that task extends `control-requirement-catalogue.yml` itself with real evidence and remains separate and not started; this is an advisory-only prompt tune, same hard rules as every other dossier. |
| 2026-09-02 | **Systematic re-check of §3.2's own evidence against the real, full scan output found 2 more real gaps, both now named (not silently left out).** (1) **New §5.7a in the design doc** — 5 of the real `unresolved-env-target` items aren't service-to-service at all; they're 3 ledger services → a shared `ledger-db` and 2 account services → a shared `accounts-db`, and even a value-correlation fix can't resolve them because no `TypedUnit` exists for either database Deployment at all (pure Postgres init-script directories, no scanned source). Needs a deterministic (no LLM) k8s-evidenced `database` node synthesis, extending the already-shipped `k8s-namespace-node-builder.ts` pattern to a new node kind — not §3.2's LLM-eligible path at all. (2) **New §3.3** — 6 relationships from the opt-in `env-soft-graph` mechanism are already sitting in the canonical `architecture.calm.json` today at confidence 20, 2 of them demonstrably wrong (`ledgerwriter→contacts`, `ledgerwriter→transactionhistory`), with zero review path — a structurally different problem from §3.2 (confirming an already-emitted fact vs. proposing a new one), so a separate Tier A class, not folded into §3.2. Both are design-only, not yet built — see `BACKLOG.md`. |
