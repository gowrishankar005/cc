...
1. **Confirmed via direct repro**: multi-document `application.yml` files using Spring's own `spring.config.activate.on-profile` were silently merged into one map regardless of profile — a profile-gated value (e.g. a real prod datasource URL) was presented as the file's unconditional default. Fixed: an on-profile-scoped document now becomes its own separate `SpringConfigFile` entry (`docProfile` field), never merged with the unconditional facts — extends T-VM-2's already-correct file-level-profile policy to the in-file-document case it hadn't covered.
2. **Confirmed via direct repro**: `server.port` set in more than one config file for the same root (a realistic pattern — base + prod override) produced non-deterministic `architecture.calm.json` output, since `attachPortInterfaces` picked "whichever evidence entry came first," which depended on `fs.readdirSync`'s filesystem-dependent directory-walk order. Fixed: emits one real `port-interface` per distinct port (never guesses a winner — same "cite both real facts" discipline the datasource extraction already uses), deduped and sorted by evidence `ref` for stable, order-independent output.
3. **Confirmed via grep**: `port-interface-builder.ts`'s docstring claimed the protocol-population mechanism was "regression-tested directly against `buildRelationships`" — false, zero tests referenced either function. Fixed by adding the real test rather than just softening the wording — a synthetic `TypedRelationship` pointing at a spring-config unit now proves `buildRelationships` returns `protocol: 'JDBC'` end-to-end through `buildCalm`.

4 new regression tests added (2 for finding #1, 1 for #2, 1 for #3) — full suite now 69/69 (was 65/65). The id-collision fix for finding #1 (`unitFileKey()`, appending `#<profile>` only when `docProfile` is set) is additive — every already-tested id shape (file-level-profile-only, no in-file on-profile) is byte-identical to before.
