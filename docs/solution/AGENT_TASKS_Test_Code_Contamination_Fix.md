...
**Post-close review (2026-08-09, same day): 1 real completeness gap found and fixed, actively hunted for, not assumed clean.** `outbound-http-detector.ts` shares `findLibraryImportEdges` with the two fixed detectors (persistence/messaging) but calls it directly rather than through the fixed `detectUnitsByImportStrategy` wrapper — it never received the `isTestPath()` filter. Confirmed via a live re-scan of the exact `fineract-provider` case: 2 of 13 real `unresolved-http-target` items were `/test/`-path contamination (`TestConfiguration.java`, `ExternalCreditBureauIntegrationWritePlatformServiceImplTest.java`). Lower severity than the original bug (this detector never creates a `TypedUnit`, so no fabricated architecture node was ever at risk) but the same class of problem — test code polluting the HITL review queue a human has to look at. Fixed: re-categorized as `TEST_CODE` (not silently dropped — the item stays visible in `ignored-items-report.json`, just correctly excluded from genuine `CROSS_DOMAIN_UNRESOLVED` review candidates). 1 new direct unit test, full suite now 80/80.
