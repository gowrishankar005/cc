...
**Status update, 2026-08-13:** E1 and E2 have both reported — the gate
condition in the sentence above is satisfied. E1 reported positive
(`T-P0-3`). E2 reported negative-with-a-clear-path, twice
(`T-P0-1` — safe-but-unproven round 1, real-conflicts round 2). E3/E4/E5 are
still outstanding; nothing in P1+ has started. Per this file's own framing,
E2 alone reporting negative would already have been a valid basis to proceed
past the gate — it did, twice, and the extension is proceeding to
`AGENT_TASKS_Ext_Layered_Recovery.md`'s `[B]`-marked tasks next (cheaper,
already-evidenced, no gate), not blocked on E3/E4/E5.
...
| **T-P0-1 E2 fact admission** | **Done, reverted twice** — `E2-graded-fact-admission-experiment.md` | T-P0-0 | Yes (with T-P0-2) | Recall ↑ at L2 measured; **must-not-detect trap holds**; R1 lock intact; `E-charge-single-L2` still fails *for the right reason* (no fabricated edge); false-positive cost stated as a number |
