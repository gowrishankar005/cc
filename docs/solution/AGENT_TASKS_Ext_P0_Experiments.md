...
| **T-P0-5 E4 catalogue-as-data stress** | **Done, shipped 2026-08-14 — MIXED result** — `E4-catalogue-as-data-stress-test.md`. NestJS GraphQL resolver support added via catalogue rows only (zero builder files touched), but doing so surfaced a real, pre-existing, generic collision bug in the shared rule matcher (`rule-schema.ts`'s `findRule()`) that silently misattributed a real Java Spring Data `@Query` annotation to a new TypeScript-only rule, flipping a real unit's kind from `database` to `service` on `apache/fineract`. Fixed generically (mechanism-level, not per-rule); verified against a direct unit test plus the real second instance. `npm test` 94/94 (93 + 1 new) | T-P0-0 | Yes | One untested framework added via catalogue row only. **Passes only with zero builder code changes** (true for builders; the shared rule-matching primitive needed a real fix first — reported precisely, not rounded to "passed") |
