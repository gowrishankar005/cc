...
| **7** | D-outbound-java | Feign / WebClient / RestTemplate → service→service | **Java** | app | **partial → catalogue-complete for the import-only shape (done 2026-08-09)** — `outbound-http-detector.ts`/`outboundHttpPass` built + live in `DEFAULT_PASSES`; catalogue now covers `RestTemplate`/`WebClient`/`OkHttpClient`/`HttpURLConnection` (the `OkHttpClient`/`HttpURLConnection` rows added and verified by a real scan of Fineract's `ExternalCreditBureauIntegrationWritePlatformServiceImpl.java` — 2 `unresolved-http-target` items, `calm validate` 0 errors, new locked regression test, suite 60/60). `@FeignClient` decorator strategy still honestly `not-implemented`, no real sample yet — separate residual, not this rank's blocker anymore. | **B-http-client done** — next real pick is whatever hard-test/design-note-gated item comes after this ladder position |
