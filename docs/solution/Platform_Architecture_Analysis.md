...
1. **Testability is the highest-leverage next investment, ahead of new features.** Every real bug this session was caught by hand; formalizing the BoA/NestJS/Fineract regression checks into an actual `npm test` (even a simple one) converts a fragile ritual into a real safety net — cheap, and the single most consequential gap this analysis found.
