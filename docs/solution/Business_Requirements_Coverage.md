...
| **BR-40** | Explicit confidence / **review status**; nothing gates until confirmed | Confidence **implemented**. Review-status vocabulary **not** (grades ≠ review status) | `T-FS-6` (depends on `T-FS-1`) | **B** | Wait for Session B |
...
| **BR-60** | Multiple techniques, explicit reconcile, no irreversible single tool | Dual-engine + structured-file + SBOM **partial**. Trust tiers stub. Contradiction **not** built. CodeQL evaluated, **not wired** | `T-FS-3`, `T-FS-4`, `T-LR-5`, `T-LR-6` | **B** owns T-FS-3. C may *use* E1b evidence for T-LR-3/4 but must not ship the engine | Do not start T-LR-5 here |
| **BR-70** | Incremental re-extract; preserve human review; flag don’t overwrite | Overrides persist. Full fact-identity merge **not** built | `T-CL-1`…`4` | — | Blocked on Session B’s T-FS-6 |
...
| **BR-110** | Per-lens fitness measured; unmeasured lenses must not gate | Architecture gold + L0–L4 **implemented**. Module gold: `threat-signals` only (`T-LM-0` **done**) | `T-LM-5`; Session A must author gold from fixture source (CON-40) | **A** (gold for any new lens) | Do not let Session A gate CI on an unscored lens |
