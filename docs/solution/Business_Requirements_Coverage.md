...
| **BR-50 — architectural-pattern conformance lens** | `calm validate -p` is explicitly *external* governance in the platform design | **Done** — `T-LM-8` (2026-08-20) decided (a) external; see `Claim_Register.md`'s "Pattern-conformance stance" section and `OOS_Registry.md`'s `OOS-pattern-conformance-engine` |
