...
**Scanning & analysis** — hybrid dual-engine scanning (per-package native typing + a combined cross-package structural pass); JAX-RS/Spring MVC/Flask/NestJS route composition; JPA/import-based persistence detection; messaging consumer/producer detection; outbound-HTTP client detection; hand-rolled resilience-logic candidate detection; OpenAPI ingestion; Spring configuration file reading; dependency-manifest (SBOM) corroboration; multi-hop bridge resolution across layered access classes; Kubernetes shared-secret trust relationships, `deployed-in` runtime-placement relationships, and manifest-vs-config contradiction detection; CloudFormation/SAM API-Gateway-to-Lambda route binding; ranked cross-repo joins against another repo's own manifest. See [What runs by default vs. what needs a flag](#what-runs-by-default-vs-what-needs-a-flag) for which of these are always-on.
...
| Dependency-manifest (SBOM) corroboration + secondary-source introduction | **Always** (opt out with `--no-cdxgen`) | No flag needed to run it — auto-detects `@cyclonedx/cdxgen` as a `pipeline` devDependency and a recognized manifest (`pom.xml`/`build.gradle`/`package.json`/etc.) per root; a no-op (not an error) when no manifest exists. **Real cost for a Java/Maven root**: cdxgen shells out to a real local `mvn dependency:tree`, not just a manifest parse — on a cold `~/.m2` this is genuine network I/O to Maven Central (measured: ~2 minutes for one Spring Boot service against an empty cache), and on a corporate network whose Maven proxy/Artifactory blocks or stalls that call, this can consume most of a "fresh" run's wall time. Bounded at a 5-minute timeout per root as of 2026-09-07 (previously unbounded); `--no-cdxgen` skips the pass entirely if your environment can't reach Maven Central at all |
| Hand-rolled resilience-logic candidate detection (Java `Thread.sleep` call sites) | **Always** | No flag — never creates a `TypedUnit`, only a dedicated `IgnoredItem` (`hand-rolled-resilience-candidate:` prefix) an architect can review via `tools/review-session/`'s Tier C flow; see `docs/solution/Architect_Residual_Review_Session.md` §3.1 |
...
| `--no-cdxgen` | No | off (cdxgen corroboration runs by default) | Skips the dependency-manifest (SBOM) corroboration pass entirely. Real, live-found need (2026-09-07): for a Java root, `cdxgen` shells out to a real local `mvn dependency:tree` against Maven Central, not just a manifest parse — on a machine whose Maven proxy/Artifactory can't reach it, this can silently eat most of a run's wall time (now bounded at a 5-minute-per-root timeout, but still real cost, still worth skipping if you know your network can't do this) |
...
- A "fresh" scan taking much longer than expected, especially with Java/Maven roots in the mix? → check whether your Maven proxy/Artifactory can actually reach Maven Central; if not, `--no-cdxgen` skips the pass that's shelling out to `mvn dependency:tree`.
