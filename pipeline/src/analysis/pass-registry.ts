...
export interface AnalysisContext {
  ...
  /** T-onboarding-26 — set by run-slice.ts's --auto-codeql detection when a Maven build also exists alongside the primary Gradle command; getOrBuildCodeqlDatabase tries this only if the primary command's build genuinely fails. Never set when --codeql-build-command was hand-written (no fallback to infer). */
