...
tools/review-session/
  ...
  preview_merge.py        §3.2 T-2 (AGENT_TASKS_Unresolved_Outbound_Target_T2_Preview_Merge.md, BACKLOG.md) — read-only preview of canonical architecture.calm.json plus current drafts/, pending relationship_add relationships tagged `x-aac-draft: true`, written to a new preview-merged.calm.json. Never touches the canonical file or drafts/ itself. Reuses apply.py's own _merge_drafts helper and the exact same run-slice.js --from-facts --overrides --out invocation a real apply uses (never a second CALM-merge implementation), just pointed at a throwaway temp --out dir, then diffed against the real canonical file to find what's genuinely new — override-applier.ts's own real-apply-only x-aac-status/x-aac-override-provenance tags are stripped from anything identified as a pending draft (misleading in a preview otherwise). Scoped to relationship_add-sourced new relationships, matching §3.2's own stated scope; other override types sitting in the same drafts/ dir are merged in by the same mechanism but not specially tagged
  test_preview_merge.py   (3 real end-to-end tests, no mocks) zero-overrides sanity check (the diff baseline must reproduce the real canonical relationship set exactly), a real drafted relationship correctly tagged and stripped of real-apply-only metadata, an already-applied override correctly NOT re-tagged as pending (apply.py never archives a draft after applying it, so drafts/ presence alone is never a valid "still pending" signal)
  ...
  dossier.py              T-2 (AGENT_TASKS_Residual_Dossier_Module.md, BACKLOG.md) — the opt-in Evidence Dossier pass, wired into pack.py behind `--with-dossier` (off by default). For every open residual (any tier — A, B, or C alike), calls the `claude` CLI to produce an explanation + hypotheses + evidenceRefsUsed, in a fixed JSON shape whose only allowed top-level keys are those three. NEVER WRITES A FACT: any response shaped like a decision/override is rejected outright, same boundary advisory.py's own parse_and_validate_response already established. Attaches the result as an additive `dossier` field on the residual in residuals.json (residuals-schema.json updated). No `claude` CLI on PATH (even with --with-dossier set) -> pack.py logs what would have been attempted and writes a normal dossier-less pack. **Real, live-found risk (2026-09-05)**: a full ~51-residual pack hit sustained rate-limiting, and a second real run consumed enough usage quota to lock the calling session out for hours -- `--dossier-limit N` (pack.py) bounds the batch to the first N open residuals instead of attempting every one; 15 is a real, evidenced safe margin (comfortably under the ~20-30 calls that succeeded before hitting a wall both times). `pack.py` only refuses to rebuild a session-dir with unapplied drafts, so packing dossier-less first and re-packing the same dir with `--with-dossier` later, once the real residual count is known, costs nothing. For `S2-http-without-security-control` specifically (AGENT_TASKS_Dossier_Uncatalogued_Auth.md, BACKLOG.md, 2026-09-02), the system prompt gets a trigger-specific addendum (`S2_AUTH_JUDGMENT_ADDENDUM`) steering the model at the exact judgment this residual raises — does the code actually contain a real auth pattern this pipeline's 4 catalogued vocabularies don't recognize — rather than a generic evidence summary. This is advisory only, same hard rules as every other dossier; it does NOT extend `control-requirement-catalogue.yml` and is not T-RT-3 (AGENT_TASKS_Ext_Review_Throughput.md), which remains a separate, not-yet-started deterministic-catalogue-extension task
...
# dossier-less pack. Real cost: $0.08-$0.32 and 40-132s per residual (measured) -- check the real
# residual count in manifest.json first (a plain `pack.py` run, no flag, gives you this for free)
# before deciding whether to run this on a large pack; --dossier-limit bounds the batch instead of
# attempting every open residual (see the CLI reference table in the top-level README).
python3 pack.py --out-dir /tmp/my-run --session-dir ../../review-sessions/my-run --with-dossier --dossier-limit 15
