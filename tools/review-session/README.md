...
4. **Apply requires an explicit, confirmed human action.** A VS Code Copilot Chat mode bound to a Session Pack must never be granted unrestricted terminal access — see `.github/chatmodes/` for the enforcement mechanism.
...
# touches drafts/. Needs ANTHROPIC_API_KEY; no key -> reports what it would attempt, writes nothing.
